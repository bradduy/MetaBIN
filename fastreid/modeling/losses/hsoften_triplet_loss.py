...
class HSoftenTripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=True, HSoft=False):
        ...
        # loss = self.ranking_loss(dist_an / 1000, y)
        # return loss, dist_an

def hsoftenTripletLoss(global_feat, labels, margin, normalize_feature=True, HSoft=False):
    if margin is not None:
    ...
    if normalize_feature:
        global_feat = normalize(global_feat, axis=-1)
    dist_mat = euclidean_dist(global_feat, global_feat)
    dist_ap, dist_an = hard_example_mining(dist_mat, labels, HSoft=HSoft)
    y = dist_an.new().resize_as_(dist_an).fill_(1)
    if margin is not None:
